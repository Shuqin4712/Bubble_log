...
- **月报**：面板内「📅 月报」，含总条数/环比、四类占比环、打卡率、最长连击、单日最高、月度排名，配一段自动生成的小结文案；可一键导出**竖版海报图**存相册或分享
...
- 单文件架构：`BubbleLog.js` 内部分区为 CONFIG / 日期工具 / Store / Stats / HeatmapPainter / RingPainter / ReportText / PosterPainter / WidgetView / ReportView / PanelView / main
...
- 月报文案接 DeepSeek API（在规则模板之上生成更有温度的小结，需保留本地兜底）
