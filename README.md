...
- **月报**：面板内「📅 月报」，含本月总数/环比、四类明细、打卡率、最长连击、单日最高、月度排名；点日历可全屏查看**四类明细大日历**（每格分开显示文字/语音/图片/表情条数）；可一键导出 1200px **竖版海报图**存相册或分享
- **每日照片**：记录面板内「📷 这天的照片」从相册选图，作为该日在月报明细日历和海报上的**格子底图**（照片上自动压暗色蒙版保证数字可读）；紧凑日历上配了照片的日子右下角有小圆点标记
...
├── avatar.png         # 组件头像（256x256，可选）
└── photos/            # 每日照片（可选）
    └── 2026-07-04.jpg # 裁成 308x280，约 30~60KB/张
```

每日照片按需存放，没配照片的日子不占空间。一年每天都配的话约 15MB。
...
- 两套日历渲染分工：`paintMonth` 每格一个总数（36pt 格子，面板日常看）；`paintMonthDetail` 每格四类明细 + 可选照片底图（154x140 格子，月报全屏图和海报用）
