...
- **桌面组件**（small / medium / large）：D+ 天数、今日四类消息计数、累计总数，粉色渐变主题，支持深浅色模式；large 档位展示 GitHub 风格的近 30 天热力格（无日期数字的纯色块）
- **快速记录面板**：点组件打开，四个大按钮「💬 文字 / 🎙 语音 / 🖼 图片 / 😝 表情」点一下 +1，可连点；支持撤销上一条、补记其他日期
- **统计**：今日明细、本月/上月/环比、历史累计、日均条数
- **月度热力图**：记录面板内为日历式布局（带日期），消息越多颜色越深（类似 GitHub contribution graph）；桌面组件上为紧凑的近 30 天色块格
